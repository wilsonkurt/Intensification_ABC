#Create a function that will take our simulated calGrids object and make it into an arrow data table to speed up processing
...
#This is a replication of the rcarbon SPD function but using the arrow table format. This function will always operate
#like normalised = TRUE is chosen in the spd function in rcarbon as we normalise all of the spds (but not all of the calibrations)
arrow_spd <- function(x, timeRange){
  
  # arrow doesn't have native support for window functions yet
  # so we convert to duck_db first 
  # (this has zero overhead because it's just passing around a C pointer)
  
  x |> 
    filter(calBP <= timeRange[1], calBP >= timeRange[2]) |> 
    to_duckdb() |> 
    group_by(calBP) |> 
    summarize(PrDens = sum(PrDens)) |> 
    mutate(PrDens = PrDens/sum(PrDens, na.rm = TRUE)) |>
    ungroup() |> 
    to_arrow() |> 
    collect() |> 
    complete(
      calBP = seq(timeRange[1], timeRange[2], by = -1), 
      fill = list(PrDens = 0)
    ) |> 
    arrange(desc(calBP)) |> 
    as.data.frame()
...
createmodelspd <- function(model) {
    ...
    
  {
    ...
    mod.results.df <- data.frame(#param.combo = param.combo,
                                 ...
                                 nrmse.cal.norm = NA,
                                 nrmse.cal.nnorm = NA,
                                 euc.weighted.uncal.norm = NA,
                                 euc.weighted.uncal.nnorm = NA,
                                 euc.weighted.cal.norm = NA,
                                 euc.weighted.cal.nnorm = NA)
    mod.results.df[,13:16016] <- NA
 ...
 # errors <- sample(dat_cal.Plat.norm$metadata$Error, size = n.samples, replace =TRUE) #generate date errors by sampling 
  ...
  #this code would use rcarbon's spd function
  #spd.uncalsample.norm.rcarbon <- spd(uncalsample.norm, timeRange = c(4000,0), spdnormalised = TRUE, verbose = FALSE)
  ...
  uncalsample.norm <- to_arrow_table(uncalsample.norm)#translate the calGrid object to an arrow table to speed up computation
  uncalsample.nnorm <- to_arrow_table(uncalsample.nnorm)#translate the calGrid object to an arrow table to speed up computation
  calsample.norm <- to_arrow_table(calsample.norm)#translate the calGrid object to an arrow table to speed up computation
  calsample.nnorm <- to_arrow_table(calsample.nnorm)#translate the calGrid object to an arrow table to speed up computation
  
  #this code uses the arrow tables from above
  spd.uncalsample.norm <-   arrow_spd(uncalsample.norm, timeRange = c(4000,0))
  spd.uncalsample.nnorm <- arrow_spd(uncalsample.nnorm, timeRange = c(4000,0))
  spd.calsample.norm <- arrow_spd(calsample.norm, timeRange = c(4000,0))
  spd.calsample.nnorm <- arrow_spd(calsample.nnorm, timeRange = c(4000,0))
  ...
  #to run these using rcarbon spd insert $grid before each call to $PrDens
  euc.uncal.norm <- data.frame()
  for (i in 2:length(observed.spds.norm)) {
    euc <- sqrt(sum((observed.spds.norm[yrs.match.range,i] - spd.uncalsample.norm$PrDens[yrs.match.range])^2))
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc <- sqrt(sum((observed.spds.nnorm[yrs.match.range,i] - spd.uncalsample.nnorm$PrDens[yrs.match.range])^2))
  ...
  for (i in 2:length(observed.spds.norm)) {
    euc <- sqrt(sum((observed.spds.norm[yrs.match.range,i] - spd.calsample.norm$PrDens[yrs.match.range])^2))
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc <- sqrt(sum((observed.spds.nnorm[yrs.match.range,i] - spd.calsample.nnorm$PrDens[yrs.match.range])^2))
  ...
  for (i in 2:length(observed.spds.norm)) {
    euc.delta <- sqrt(sum(((observed.spds.norm[yrs.match.range,i] - spd.uncalsample.norm$PrDens[yrs.match.range]) / 
                         (spd.uncalsample.norm$PrDens[yrs.match.range] + delta.val))^2))
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc.delta <- sqrt(sum(((observed.spds.nnorm[yrs.match.range,i] - spd.uncalsample.nnorm$PrDens[yrs.match.range]) / 
                             (spd.uncalsample.nnorm$PrDens[yrs.match.range] + delta.val))^2))
  ...
  for (i in 2:length(observed.spds.norm)) {
    euc.delta <- sqrt(sum(((observed.spds.norm[yrs.match.range,i] - spd.calsample.norm$PrDens[yrs.match.range]) / 
                             (spd.calsample.norm$PrDens[yrs.match.range] + delta.val))^2))
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc.delta <- sqrt(sum(((observed.spds.nnorm[yrs.match.range,i] - spd.calsample.nnorm$PrDens[yrs.match.range]) / 
                             (spd.calsample.nnorm$PrDens[yrs.match.range] + delta.val))^2))
  ...
  for (i in 2:length(observed.spds.norm)) {
   euc.weighted <-  sqrt((1/sum(weights.vec) *
                       (sum(
                         weights.vec[i] * (observed.spds.norm[yrs.match.range,i] - spd.uncalsample.norm$PrDens[yrs.match.range])^2
                        ))
                      )
                    )
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc.weighted <-  sqrt((1/sum(weights.vec) *
                             (sum(
                               weights.vec[i] * (observed.spds.nnorm[yrs.match.range,i] - spd.uncalsample.nnorm$PrDens[yrs.match.range])^2
                             ))
    )
    )
  ...
  for (i in 2:length(observed.spds.norm)) {
    euc.weighted <-  sqrt((1/sum(weights.vec) *
                             (sum(
                               weights.vec[i] * (observed.spds.norm[yrs.match.range,i] - spd.calsample.norm$PrDens[yrs.match.range])^2
                             ))
    )
    )
  ...
  for (i in 2:length(observed.spds.nnorm)) {
    euc.weighted <-  sqrt((1/sum(weights.vec) *
                             (sum(
                               weights.vec[i] * (observed.spds.nnorm[yrs.match.range,i] - spd.calsample.nnorm$PrDens[yrs.match.range])^2
                             ))
    )
    )
  ...
  mod.results.df[,13:4013] <- spd.uncalsample.norm$PrDens
  mod.results.df[,4014:8014] <- spd.uncalsample.nnorm$PrDens
  mod.results.df[,8015:12015] <- spd.calsample.norm$PrDens
  mod.results.df[,12016:16016] <- spd.calsample.nnorm$PrDens
