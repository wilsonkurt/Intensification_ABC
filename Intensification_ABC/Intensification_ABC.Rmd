...
clim.df <- read.csv("./data/clim.df.csv")#read the 21ka climate data with npp calculated using Miami model
clim.df <- clim.df[which(clim.df$age < 4001 & clim.df$age > -1),]#cut to the past 4000 years
clim.df <- approx(clim.df$age, clim.df$npp, xout = seq(4000,0,-1))#interpolate npp between each of the decadal reconstructions for annual resolution
clim.df <- data.frame(age = clim.df$x, npp = clim.df$y)

#read in the saved spds
observed.spds <- read.csv("./data/Observed_spds_Plateau.csv")
observed.spds.norm <- observed.spds[,c(1,seq(2,2001,2))]#grab the normalized spds and put in own object
observed.spds.nnorm <- observed.spds[,c(1,seq(3,2002,2))]#make a df for the not normalized spds

observed.spds.basin <- read.csv("./data/Observed_spds_Basin.csv")
observed.spds.basin.norm <- observed.spds.basin[,c(1,seq(2,2001,2))]#grab the normalized spds and put in own object
observed.spds.basin.nnorm <- observed.spds.basin[,c(1,seq(3,2002,2))]#make a df for the not normalized spds
...
par(mfrow = c(1,2))
plot(1,ylab = "Summed Probability (Normalized)", xlab = "CalBP", ylim = c(0, 0.0008), xlim = c(0,4000))
polygon(x = c(1000,1000,4000,4000), y = c(0,0.0008,0.0008,0), col = alpha("grey", alpha = 0.5), border = NA)
for (i in 2:1001) {
  lines(observed.spds.norm[,i]~observed.spds.norm$calBP, col = rgb(59, 15, 112, max = 255, alpha = 10))
}
...
plot(1, ylab = "Summed Probability (not Normalized)", xlab = "CalBP", ylim = c(0, 0.0008), xlim = c(0,4000))
polygon(x = c(1000,1000,4000,4000), y = c(0,0.0008,0.0008,0), col = alpha("grey", alpha = 0.5), border = NA)
for (i in 2:1001) {
  lines(observed.spds.nnorm[,i]~observed.spds.norm$calBP, col = rgb(96,206,172, max = 255, alpha = 10))
}
#lines(x = c(1000,1000), y = c(0,0.0008), lty = 2, col = alpha("grey20", alpha = 0.5))
#lines(x = c(4000,4000), y = c(0,0.0008), lty = 2, col = alpha("grey20", alpha = 0.5))
...
#Globals
nsim <- 5 #set the number of parameter combinations to use
...
plot.df <- data.frame(npp = c(min(clim.df$npp), params$npp_lo[4], params$npp_hi[4], max(clim.df$npp)),
