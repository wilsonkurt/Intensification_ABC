...
observed.spds <- read.csv("Observed_spds_1000reps.csv")
```
...
no.dups[which(no.dups$combo == 1),]

```


Next, with the simulated model, we need to generate a summed probability distribution. The code below (along with the function in the simulatedmodelspds source file) allow us to do this. The spd recreation code is mostly replicated from Dinapoli et al 2021.
```{r}
#setup for parallel processing & tracking progress
ncores <- 10
cl <- makeCluster(ncores)
...
sim.spds <- foreach (i=55:56, .packages = c("rcarbon"), .options.snow = opts) %dopar%
  {
    #subset to a single model simulation run
    model <- no.dups[which(no.dups$combo == i),]
    ...
    #conver the model dataframe into a CalGrid object
    class(model) <- 'CalGrid'
...
set.seed(i)
    model.spds <- createmodelspd(model=model)
    return(model.spds)
