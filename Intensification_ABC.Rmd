...
plot(NULL,xlim = c(0, max(results$CalBP)), ylim = c(0, max(results$N, na.rm = TRUE)), ylab = "Population Size", xlab = "CalBP")
for (i in 1:nsim){
  lines(subset(results$N, results$combo == i) ~ subset(results$CalBP, results$combo == i), lwd = 1, col = alpha("grey50", alpha = 0.5))
}
...
sim.spds <- foreach (i=25:25, .packages = c("rcarbon"), .options.snow = opts) %dopar%
  {
    #subset to a single model simulation run
    model <- results[which(results$combo == i),]
